...
for k in range(nfolds):
    ...
    k2 = k+1
    print(str(nfolds)+ '-fold CV, ' +'Running Neural Net round '+str(k2)+'...')
    
    X = np.array(train[k])

    Y = np.array(trainLab2[k])

    #One vs Rest (multiclass) MLP Classifier
    clf = OneVsRestClassifier(MLPClassifier(solver='adam', alpha=1e-4, activation = 'tanh', hidden_layer_sizes = (100,100,100), random_state=1,learning_rate='constant',learning_rate_init=0.0001,max_iter=10000))
...
#Save MLP prediction results
DF2 = pd.concat(DF)
DF2.to_csv('pydat/results/MLP/Testing_ALS451_MLP_results_3layer100Units_100CV_OA1000_adam_tanh.csv')
#Save MLP classifier for Holdout analysis
dump(clf,'MLP_OA1000_adam_tanh.joblib')
#Save the most recent Testing Cohort ROC plot 
fig.savefig('pydat/ROC/NovaSeq_Test_ROC_MLP_OA1000_adam_tanh.svg')
