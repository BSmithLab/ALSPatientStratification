...
#To obtain Rank Metric Scores from GSEA
#1) Set "Plot graphs for the top sets of each phenotype" to a large number (ensure this is equal to the number of enriched pathways)
#2) Locate the GSEA results folder, and find the .TSV file corresponding to the enrichment pathway you are interested in
#3) Gene symbol and rank metric score entries were retained and used in the analysis below
...
TE = read.csv("TE_Alz_Heatmap.csv") #File generated from GSEA (see material/methods section and comment above)
...
colnames(tmp)= c("Control","ALS-GLIA","ALS-TD","ALS-OX")


mypalette = rev(colorRampPalette(brewer.pal(10, "RdBu"))(18))
heatmap.2(tmp,scale="none",col=mypalette,trace="none",density.info="none",key.xlab = "GSEA Rank Metric Score",dendrogram = "row",hclustfun = function(x) hclust(x,method = 'median'),ylab="Gene",labRow = rownames(tmp),labCol = colnames(tmp),margins=c(8,8),cexRow = 0.65,cexCol = 1.25,Colv = F)


##############################################################################################################################################################

#Genes upregulated in Alzheimer's
...
mypalette = rev(colorRampPalette(brewer.pal(10, "RdBu"))(18))
heatmap.2(tmp,scale="none",col=mypalette,trace="none",density.info="none",key.xlab = "GSEA Rank Metric Score",dendrogram = "row",hclustfun = function(x) hclust(x,method = 'median'),xlab = "ALS Subtype",labRow = rownames(tmp),labCol = colnames(tmp),margins=c(8,8),cexRow = 1.3,cexCol = 1.25,Colv = F)
...
mypalette = rev(colorRampPalette(brewer.pal(10, "RdBu"))(18))
heatmap.2(tmp,scale="none",col=mypalette,trace="none",density.info="none",key.xlab = "GSEA Rank Metric Score",dendrogram = "row",hclustfun = function(x) hclust(x,method = 'median'),xlab = "ALS Subtype",labRow = rownames(tmp),labCol = colnames(tmp),margins=c(8,8),cexRow = 1.2,cexCol = 1.25,Colv = F)
...
setwd("C:/Users/jeshima/Documents/Smith Lab/Summer 2021/ALS Subtyping/GSEA/Final/ManualHeatmaps/Innate")

TE = read.csv("TE_Innate_Heatmap.csv") #File generated from GSEA 
OX = read.csv("OX_Innate_Heatmap.csv") #File generated from GSEA 
GLIA = read.csv("GLIA_Innate_Heatmap.csv") #File generated from GSEA 
HC = read.csv("HC_Innate_Heatmap.csv") #File generated from GSEA 

Innate_heatmap = data.frame(matrix(NA,nrow=nrow(TE),ncol=4))
rownames(Innate_heatmap) = TE$SYMBOL
colnames(Innate_heatmap) = c("HC","GLIA","TE","OX")
...
Innate_heatmap$TE = TEscore
Innate_heatmap$OX = OXscore
Innate_heatmap$GLIA = GLIAscore
Innate_heatmap$HC = HCscore


Innate_heatmap2 = matrix(as.numeric(unlist(Innate_heatmap),ncol = 4))
tmp = matrix(Innate_heatmap2,ncol=4)
typeof(tmp)
rownames(tmp) = TE$SYMBOL
colnames(tmp)= c("Control","ALS-GLIA","ALS-TD","ALS-OX")
...
#Without labels
heatmap.2(TE_heatmap_zscore,scale="none",Colv = F,col=mypalette,trace="none",density.info="none",key.xlab = "Z-Score Counts",dendrogram = "row",hclustfun = function(x) hclust(x,method = 'median'),labRow = NA,labCol = colnames(tmp),margins=c(8,8),cexRow = 0.2,cexCol = 0.15)

#With labels (extremely small font)
