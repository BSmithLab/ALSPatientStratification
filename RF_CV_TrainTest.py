...
##This is a script to perform sklearn random forest for supervised classification problems
...
for k in range(nfolds):
    ...
    k2 = k+1
    print(str(nfolds)+ '-fold CV, ' +'Running RF round '+str(k2)+'...')
    
    X = np.array(train[k])

    Y = np.array(trainLab2[k])

    clf = OneVsRestClassifier(RandomForestClassifier(n_estimators=1000, oob_score=True,class_weight=(weights)))
...
DF2 = pd.concat(DF)
DF2.to_csv('pydat/results/RFpy/Testing_ALS451_RF_results_ntree1000_100CV_OA1000_AllWeights.csv')

dump(clf,'RF_OA1000_ntree1000_AllWeights.joblib')

fig.savefig('pydat/ROC/NovaSeq_Test_ROC_RF_ntree1000_OA1000_AllWeights.svg')
