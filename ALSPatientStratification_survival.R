...
plot(km_fit,col = c("darkorchid2","goldenrod","navy","firebrick"),main="ALS Subtype Kaplan-Meier Survival",xlab="Disease Duration (months)",ylab="Survival Probability",lwd=2,cex.axis=1.5,cex.main=1.5,cex.lab=1.5)
legend(130,1,legend=c("Discordant","ALS-Ox","ALS-Glia","ALS-TE"),lty = 1,lwd = 2.5,col=c("darkorchid2","navy","goldenrod","firebrick"))
text(10,0.2,"p = 0.023",cex=2)

abline(h=0.5,lty="dashed")
abline(v=28,lty="dashed")
abline(v=36,lty="dashed")
abline(v=42,lty="dashed")

ggsurvplot(km_fit,pval = F,conf.int = F,risk.table = F,linetype = "strata",surv.median.line = "hv",ggtheme = theme_bw(),palette = c("goldenrod","navy","firebrick","darkorchid2"))
...
ggsurvplot(
  ...
  break.time.by = 24,     # break X axis in time intervals by 200.
  ...
  legend.labs =
    c("Discordant","GLIA", "OX","TE"),    # change legend labels.
  palette =
    c("darkorchid2","goldenrod","navy","firebrick") # custom color palettes.
)
...
## Considering only subjects with agreement between motor and frontal cortex samples
...
legend(140,1.02,legend=c("ALS-Glia","ALS-Ox","ALS-TD"),lty = 1,lwd=3.5,col=c("goldenrod","navy","firebrick"))
...
ggsurvplot(km_fit,pval = F,conf.int = F,risk.table = F,linetype = "strata",surv.median.line = "hv",ggtheme = theme_bw(),palette = c("goldenrod","navy","firebrick"))
...
ggsurvplot(
  ...
  surv.median.line = "hv",  # add the median survival pointer.
  legend.labs =
    c("GLIA", "OX","TE"),    # change legend labels.
  palette =
    c("goldenrod","navy","firebrick") # custom color palettes.
)
