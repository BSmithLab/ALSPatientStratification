...
##This is a script to perform sklearn linear support vector machine classifier for supervised classification problems
...
#Define multiclass weights
weights = dict([{0:4.2,1:1},{0:0.81,1:1},{0:2.9,1:1}]) #Train/Test Weights
...
for k in range(nfolds):
    ...
    k2 = k+1
    print(str(nfolds)+ '-fold CV, ' +'Running LinearSVC round '+str(k2)+'...')
    
    X = np.array(train[k])

    Y = np.array(trainLab2[k])

    Classifier = OneVsRestClassifier(LinearSVC(max_iter = 100000,C=1.0,class_weight=(weights)))
...
DF2 = pd.concat(DF)
DF2.to_csv('pydat/results/SVM/Testing_ALS451_SVM_results_100CV_OA1000_Calibrated_Trainweights_100k.csv')

dump(clf,'SVM_OA1000_Calibrated_Trainweights_100k.joblib')

fig.savefig('pydat/ROC/NovaSeq_Test_ROC_SVM_OA1000_Calibrated_Trainweights_100k.svg')
