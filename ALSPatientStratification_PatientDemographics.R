...
TEaoo = as.numeric(TD$Age.at.Symptom.Onset)
...
par(mar = c(5, 5, 3, 2))
boxplot(GLaoo,OXaoo,TEaoo,xaxt="n",main=c("Subtype Age of Onset"),cex.axis = 1.5,cex.main=1.5,col=c("goldenrod1","navy","firebrick"),xlab="Subtype",ylab="Age of Onset (years)",cex.lab=1.5,pch=20,cex=1.4,ylim=c(25,85))
...
TEaod = as.numeric(TD$Age.at.Death)
...
par(mar = c(5, 5, 3, 2))
boxplot(GLaod,OXaod,TEaod,xaxt="n",main=c("Subtype Age of Death"),cex.axis = 1.5,cex.main=1.5,col=c("goldenrod1","navy","firebrick"),xlab="Subtype",ylab="Age of Death (years)",cex.lab=1.5,pch=20,cex=1.4,ylim=c(30,90))
axis(at=1:3,side=1,labels=c("ALS-Glia","ALS-Ox","ALS-TD"),cex.axis=1.5)
...
TE.FTD = names(table(TD$Subject.Group.Subcategory))
blank = rep(NA,nrow(TD))
for(j in 1:nrow(TD)){
  if(TD$Subject.Group.Subcategory[j] == TE.FTD[6]){
    blank[j] = "Positive"
  }else{
    blank[j] = "Negative"
  }
