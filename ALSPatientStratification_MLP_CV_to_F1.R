...
for(i in 1:length(list)){
  tmp = read.csv(paste("ALS451_MLP_results_3layer100Units_100CV_",list[i],".csv",sep="")) #File generated in Python scripts
  assign(list[i],tmp)
  rm(tmp)
}
...
for(i in 1:length(list)){
  ...
  report[,6] = "MLP"
  report[,1] = c("",rep(c("GLIA","OX","TE"),nfold)) #may have to change the order manually depending on future reports
  ...
  file = paste(list[i],"_report_MLP_100CV_70_30.csv")
